...
# [Step 0] 스마트 런처 (엔진 업그레이드: fpdf -> fpdf2)
def setup_environment():
    # 1. 라이브러리 설치 (fpdf2 강제 설치)
    try:
        import fpdf
        # fpdf2인지 확인 (fpdf2는 __version__ 속성이 있음, 구형은 없을 수 있음)
        if not hasattr(fpdf, '__version__'):
            raise ImportError
    except ImportError:
        print("🛠️ [시스템] PDF 엔진 업그레이드 중 (fpdf -> fpdf2)...")
...
# --------------------------------------------------------------------------------
# [Engine 1] 한글 PDF 생성기 (Upgrade: fpdf2)
# --------------------------------------------------------------------------------
class PDF(FPDF):
    def header(self):
        font_path = 'NanumGothic.ttf'
        if os.path.exists(font_path):
            # fpdf2 방식: uni=True 불필요
...
def generate_korean_pdf(address, context):
    ...
    return pdf.output(dest='S') # fpdf2는 bytes 반환
...
with tab1:
    ...
    with col_d2:
        st.caption("👈 **[버그 수정 완료]** 이제 한글이 완벽하게 지원되는 정밀 보고서를 다운로드할 수 있습니다.")
